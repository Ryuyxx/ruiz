import Layout from "../components/Layout";
import Image from "next/image";

export default function Home() {
  return (
      <Layout title="Home">
          <div className="bg-white text-center shadow-xl p-8 w-80 rounded-2xl dark:bg-gray-800 ">
              <div className="mt-4">
                  <p className="font-bold">Myself</p>
              </div>
              <div className="flex justify-center mt-4">
                  <Image
                      className="rounded-full absolute inset-0 transform hover:-translate-x-20 transition duration-300 cursor-pointer"
                      src="/profile/S__31137960.jpg"
                      width={60}
                      height={60}
                      alt="Avatar"
                  />
              </div>
              <div className="mt-4">
                  <p className="font-bold">Name</p>
                  <p className="text-xs mt-1 mb-1 text-blue-400">Hoi Ching Chan</p>
                  <p className="font-bold">Languages</p>
                  <p className="text-xs mt-1 mb-1 text-gray-600">English, Chinese, Cantonese, Japanese</p>
                  <p className="font-bold">Location</p>
                  <p className="text-xs mt-1 mb-1 text-gray-600">Tokyo, Japan</p>
                  <p className="font-bold">E-mail</p>
                  <p className="text-xs mt-1 mb-1 text-gray-600">ruizu@gmail.com</p>

              </div>
              <div className="mt-6 flex justify-around">
                  <div>
                      <a
                          href="https://www.instagram.com/hoichingchan/"
                          target="_blank"
                          rel="noopener noreferrer"
                      >
                          <svg className="w-6 h-6 mr-3 text-black-500 inset-0 transform  hover:scale-75 transition duration-300"
                               fill="currentColor" viewBox="0 0 20 20">
                              <path d="M14.52,2.469H5.482c-1.664,0-3.013,1.349-3.013,3.013v9.038c0,1.662,1.349,3.012,3.013,3.012h9.038c1.662,0,3.012-1.35,3.012-3.012V5.482C17.531,3.818,16.182,2.469,14.52,2.469 M13.012,4.729h2.26v2.259h-2.26V4.729z M10,6.988c1.664,0,3.012,1.349,3.012,3.012c0,1.664-1.348,3.013-3.012,3.013c-1.664,0-3.012-1.349-3.012-3.013C6.988,8.336,8.336,6.988,10,6.988 M16.025,14.52c0,0.831-0.676,1.506-1.506,1.506H5.482c-0.831,0-1.507-0.675-1.507-1.506V9.247h1.583C5.516,9.494,5.482,9.743,5.482,10c0,2.497,2.023,4.52,4.518,4.52c2.494,0,4.52-2.022,4.52-4.52c0-0.257-0.035-0.506-0.076-0.753h1.582V14.52z" />
                          </svg>
                      </a>
                  </div>
                  {/*<div>*/}
                  {/*    <a*/}
                  {/*        href=""*/}
                  {/*        target="_blank"*/}
                  {/*        rel="noopener noreferrer"*/}
                  {/*    >*/}
                  {/*        <svg className="w-6 h-6 mr-3 text-black-500"*/}
                  {/*             fill="currentColor" viewBox="0 0 20 20">*/}
                  {/*            <path d="M18.258,3.266c-0.693,0.405-1.46,0.698-2.277,0.857c-0.653-0.686-1.586-1.115-2.618-1.115c-1.98,0-3.586,1.581-3.586,3.53c0,0.276,0.031,0.545,0.092,0.805C6.888,7.195,4.245,5.79,2.476,3.654C2.167,4.176,1.99,4.781,1.99,5.429c0,1.224,0.633,2.305,1.596,2.938C2.999,8.349,2.445,8.19,1.961,7.925C1.96,7.94,1.96,7.954,1.96,7.97c0,1.71,1.237,3.138,2.877,3.462c-0.301,0.08-0.617,0.123-0.945,0.123c-0.23,0-0.456-0.021-0.674-0.062c0.456,1.402,1.781,2.422,3.35,2.451c-1.228,0.947-2.773,1.512-4.454,1.512c-0.291,0-0.575-0.016-0.855-0.049c1.588,1,3.473,1.586,5.498,1.586c6.598,0,10.205-5.379,10.205-10.045c0-0.153-0.003-0.305-0.01-0.456c0.7-0.499,1.308-1.12,1.789-1.827c-0.644,0.28-1.334,0.469-2.06,0.555C17.422,4.782,17.99,4.091,18.258,3.266" />*/}
                  {/*        </svg>*/}
                  {/*    </a>*/}
                  {/*</div>*/}
                  {/*<div>*/}
                  {/*    <a*/}
                  {/*        href=""*/}
                  {/*        target="_blank"*/}
                  {/*        rel="noopener noreferrer"*/}
                  {/*    >*/}
                  {/*        <svg className="w-6 h-6 mr-3 text-black-500" viewBox="0 0 20 20">*/}
                  {/*            <path d="M15.77,3.488c-2.266-0.067-3.801,1.105-4.605,3.519c0.415-0.158,0.817-0.237,1.206-0.237c0.829,0,1.193,0.428,1.097,1.284c-0.049,0.52-0.414,1.275-1.097,2.268c-0.682,0.993-1.192,1.489-1.534,1.489c-0.439,0-0.841-0.767-1.206-2.3C9.508,9.06,9.288,7.909,8.972,6.06C8.679,4.346,7.899,3.545,6.633,3.658c-0.537,0.045-1.34,0.496-2.412,1.354c-0.78,0.654-1.572,1.308-2.375,1.962l0.767,0.914c0.73-0.474,1.157-0.711,1.279-0.711c0.56,0,1.084,0.812,1.571,2.436c0.439,1.489,0.878,2.979,1.316,4.468c0.658,1.624,1.462,2.436,2.412,2.436c1.535,0,3.412-1.33,5.628-3.992c2.144-2.549,3.253-4.557,3.326-6.023C18.242,4.537,17.451,3.534,15.77,3.488" />*/}
                  {/*        </svg>*/}
                  {/*    </a>*/}
                  {/*</div>*/}
              </div>
          </div>

      </Layout>
  );
}